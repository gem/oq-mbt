...
mbt_importers.add(Importer('mbt.importers.faults.get_fmg_faults',
                           'get_fmg_faults', 'FMG-shapefile-shallow_faults'))
mbt_importers.add(Importer('mbt.importers.catalogues.get_hmtk_catalogue',
                           'get_htmk_catalogue', 'HMTK-csv-catalogue'))
mbt_importers.add(Importer('mbt.importers.areas.areas_to_oqt_sources',
                           'areas_to_oqt_source', 'MBT-shapefile-area'))
mbt_importers.add(Importer('mbt.importers.faults.get_oq_shp_faults',
                           'get_oq_shp_faults',
                           'MBT-shapefile-shallow_faults'))
...
class Resource_external_file(Dictable):
    ...
    def __init__(self, key, filename, loader, onthefly, checksum, mtime):
        ...
        if (len(self.filename) > 45):
            file_label = "%s ... %s" % (self.filename[0:10],
                                        self.filename[-30:])
        else:
    ...
    def add_cb(cls, btn):
        ...
        wid_onthefly_desc = widgets.HTML(
            value=("Load on-the-fly when required"
                   " (persistent and cached otherwise)"))
        wid_onthefly_box = widgets.HBox(children=[wid_onthefly,
                                                  wid_onthefly_desc])
        ...
        def new_cb(btn):
            ...
            wid_file_exists.value = (
                os.path.exists(abs_filename) and not
                os.path.isdir(abs_filename))

            if ctx.wid_file_exists.value is False:
                ctx.wid_message.value = ("File '%s' doesn't exists"
                                         % ctx.wid_filename.value)
                return False

            importer = mbt_importers.ref_by_code(ctx.wid_importer.value)
            if importer is None:
                ctx.wid_message.value = ("Importer '%s' not found."
                                         % ctx.wid_importer.value)
                return False
            ...
            res_ef = cls(ctx.wid_key.value, ctx.wid_filename.value,
                         ctx.wid_importer.value, ctx.wid_onthefly.value,
                         checksum, mtime)
            res_ef.parent_set(parent_ctx)
            parent_ctx.resource_add(res_ef)

        wid_new_btn = widgets.Button(description='Add', margin="8px")
        wid_new_btn._gem_ctx = Bunch(
            wid_message=wid_message, wid_key=wid_key,
            wid_filename=wid_filename, wid_file_exists=wid_file_exists,
            wid_importer=wid_importer, wid_onthefly=wid_onthefly,
        ...
        wid_box = widgets.Box(
            children=[wid_key, wid_title, wid_message, wid_flexbox,
                      wid_importer, wid_onthefly_box, wid_btnbox],
...
class Resource_kv(Dictable):
    ...
    def add_cb(btn):
        ...
        wid_box = widgets.Box(
            children=[wid_title, wid_name, wid_value, wid_btnbox],
            border_style="solid", border_width="1px",
...
class Resources(Dictable):
    ...
    def __init__(self, resources=[]):
        ...
        self.wid_addext = widgets.Button(
            description='Add external file', margin="8px")
